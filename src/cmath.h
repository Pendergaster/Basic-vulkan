...
static inline void mat4_mult_mat4 (mat4* restrict res,const mat4* restrict lhv,const mat4* restrict rhv)
{
    for (int y = 0; y < 4; y++)
    {
        for (int x = 0; x < 4; x++)
...
static inline void mat4_mult_mat4_inside(mat4* restrict lhv,const mat4* restrict rhv) {
    mat4 temp = *lhv;
    for (int y = 0; y < 4; y++)
    {
        for (int x = 0; x < 4; x++)
...
static inline void scale_mat4(mat4* mat,float scale) {
    for (int x = 0; x < 3; x++)
        for (int y = 0; y < 3; y++)
            mat->mat[x][y] *= scale;
}
...
static inline void create_scaling_mat4(mat4* m,const vec3 v) {
    identify_mat4(m);
    m->mat[0][0] = v.x;
    m->mat[1][1] = v.y;
    m->mat[2][2] = v.z;
